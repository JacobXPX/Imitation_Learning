...
"""
Code to train a behavioral cloning.
Example usage:
...
def main():
    ...
    if not os.path.isfile('experts/' + args.envname + '_expert.pkl'):
        expert_data = load_policy.get_expert_data(
            envname = args.envname, max_timesteps = 1000, num_rollouts = 100, render = False, save = True
            )
    ...
    my_graph = GraphGenerator(size=[256, 256, 64], bbeta=0.0001, dim_ao = dim_ao, learning_rate = 0.0005)
    bc_policy = BCPolicy(bc_graph = my_graph, train_steps = args.train_steps)
    bc_policy.fit(expert_data)
    bc_policy.save('trained_model')
