...
def save_expert_data(expert_data, file_name='Humanoid-v1'):
    """Save generated expert data to file.
    Args:
        expert_data (dict): generated expert data.
    """
    with open('experts/' + file_name+'_expert.pkl', 'wb') as f:
        pickle.dump(expert_data, f)

def get_expert_data(envname = 'Humanoid-v1', max_timesteps = 1000, num_rollouts = 100, render = False, save = False):

    print('loading and building expert policy')
    policy_fn = load_policy('experts/{}.pkl'.format(envname))
    ...
    if save:
        save_expert_data(expert_data, envname)
    return expert_data
